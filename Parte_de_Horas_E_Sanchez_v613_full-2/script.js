...
/* Parte de Horas v6.13 — full app */
const STORAGE_KEY="parte_horas_v613";
...
/* ===== Bloqueo de mes ===== */
function monthKey(state){ return `y${state.view.year}m${state.view.month}`; }
function isMonthLocked(state){ const key=monthKey(state); const locks=(state.locks||{}); return !!locks[key]; }
function setMonthLocked(state,val){ const key=monthKey(state); state.locks=state.locks||{}; state.locks[key]=!!val; saveState(state); }
...
function loadState(){
  ...
  if(!raw){
    ...
    const state={users,data:{},locks:{},view:{year:now.getFullYear(),month:now.getMonth()},currentUser:null};
    localStorage.setItem(STORAGE_KEY,JSON.stringify(state));
    return state;
  }
  const s=safeParse(raw,null); if(!s){ localStorage.removeItem(STORAGE_KEY); return loadState(); }
  s.data=s.data||{}; s.locks=s.locks||{}; s.view=s.view||{year:new Date().getFullYear(),month:new Date().getMonth()};
  return s;
}
...
function ensureModalEditor(){
  let modal=document.getElementById("obsModalEditor");
  if(modal) return modal;
  modal=document.createElement("div"); modal.id="obsModalEditor";
  modal.style.position="fixed"; modal.style.inset="0"; modal.style.background="rgba(0,0,0,.45)";
  modal.style.display="none"; modal.style.alignItems="center"; modal.style.justifyContent="center"; modal.style.zIndex="10000";
  modal.innerHTML=`<div style="background:#fff;border-radius:12px;max-width:720px;width:94%;padding:16px;border:1px solid #e5e7eb">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
      <div id="obsEdTitle" style="font-weight:800"></div>
      <div style="display:flex;gap:8px">
        <button id="obsEdDelete" class="ghost">Eliminar</button>
        <button id="obsEdSave" class="ghost">Guardar</button>
        <button id="obsEdClose" class="ghost">Cerrar</button>
      </div>
    </div>
    <textarea id="obsEdBody" style="width:100%;min-height:180px;border:1px solid #d1d5db;border-radius:8px;padding:10px;line-height:1.45;white-space:pre-wrap"></textarea>
  </div>`;
  document.body.appendChild(modal);
  modal.addEventListener("click",(e)=>{ if(e.target===modal) modal.style.display="none"; });
  return modal;
}
function showObsModalEditor(title, initialText, onSave, onDelete){
  const m=ensureModalEditor();
  m.querySelector("#obsEdTitle").textContent = title || "Observación";
  const ta=m.querySelector("#obsEdBody"); ta.value = initialText || "";
  m.style.display="flex";
  const close=()=>{ m.style.display="none"; };
  m.querySelector("#obsEdClose").onclick = close;
  m.querySelector("#obsEdSave").onclick  = ()=>{ onSave?.(ta.value); close(); };
  m.querySelector("#obsEdDelete").onclick= ()=>{ onDelete?.(); close(); };
}
...
function timesheetTable(state,user,{showObservations=false, obsMode='admin'}={}){
  ...
  for(let d=1; d<=days; d++){
    ...
    td1.appendChild(Select({value:row.ent1,onChange:v=>{row.ent1=v; sheet.days[key]=row; update();}, disabled:isMonthLocked(state)}));
    td2.appendChild(Select({value:row.salComida,onChange:v=>{row.salComida=v; sheet.days[key]=row; update();}, disabled:isMonthLocked(state)}));
    td3.appendChild(Select({value:row.ent2,onChange:v=>{row.ent2=v; sheet.days[key]=row; update();}, disabled:isMonthLocked(state)}));
    td4.appendChild(Select({value:row.sal,onChange:v=>{row.sal=v; sheet.days[key]=row; update();}, disabled:isMonthLocked(state)}));
    const res=computeDailyTotal(row.ent1,row.salComida,row.ent2,row.sal);
    tdT.textContent=res.minutes==null?"—":hhmm(res.minutes);
    tr.classList.toggle("fiesta",!!res.fiesta); tr.classList.toggle("incap",!!res.incap);
    ...
    if(showObservations){
      ...
      if(obsMode==='admin'){
        ...
        const ta=document.createElement('textarea'); ta.className='obs-ta'; ta.value=row.obs||''; ta.rows=1; ta.placeholder='Escribe una observación...';
        const autoresize=()=>{ ta.style.height='auto'; ta.style.height=(ta.scrollHeight+2)+'px'; };
        ['input','change'].forEach(evt=>ta.addEventListener(evt,autoresize)); setTimeout(autoresize,0);
        const actions=document.createElement('div'); actions.className='no-print';
        const btnSave=document.createElement('button'); btnSave.className='ghost'; btnSave.textContent='Guardar';
        const btnDel=document.createElement('button'); btnDel.className='ghost'; btnDel.style.marginLeft='6px'; btnDel.textContent='Eliminar';
        const btnEdit=document.createElement('button'); btnEdit.className='ghost'; btnEdit.style.marginLeft='6px'; btnEdit.textContent='Ver/Editar';
        const applyLockUI=()=>{ const locked=isMonthLocked(state); ta.disabled=locked; btnSave.disabled=locked; btnDel.disabled=locked; btnEdit.disabled=false; };
        applyLockUI();
        btnSave.addEventListener('click',()=>{ if(isMonthLocked(state)) return; row.obs=ta.value; sheet.days[key]=row; saveState(state); btnSave.textContent='Guardado'; setTimeout(()=>btnSave.textContent='Guardar',900); });
        btnDel.addEventListener('click',()=>{ if(isMonthLocked(state)) return; ta.value=''; autoresize(); row.obs=''; sheet.days[key]=row; saveState(state); });
        btnEdit.addEventListener('click',()=>{
          const locked=isMonthLocked(state);
          const dateLabel = new Date(state.view.year,state.view.month,d).toLocaleDateString('es-ES');
          showObsModalEditor(`${dateLabel} — Observación de ${user.name}`, ta.value,
            (newVal)=>{ if(locked) return; ta.value=newVal; autoresize(); row.obs=newVal; sheet.days[key]=row; saveState(state); },
            ()=>{ if(locked) return; ta.value=''; autoresize(); row.obs=''; sheet.days[key]=row; saveState(state); }
          );
        });
        actions.append(btnSave,btnDel,btnEdit); wrap.append(ta,actions); tdObs.appendChild(wrap);
      } else {
...
function adminView(state){
  ...
  // Header con controles de cierre
  const hdrExtra = document.createElement("div"); hdrExtra.className = "no-print"; hdrExtra.style.marginLeft = "8px";
  const btnLock = document.createElement("button"); btnLock.className="ghost"; btnLock.id="btnLockMonth";
  const btnUnlock = document.createElement("button"); btnUnlock.className="ghost"; btnUnlock.id="btnUnlockMonth";
  function renderLockButtons(){ const locked=isMonthLocked(state); btnLock.textContent="Finalizar mes"; btnUnlock.textContent="Reabrir mes"; btnLock.style.display = locked ? "none" : "inline-block"; btnUnlock.style.display = locked ? "inline-block" : "none"; }
  btnLock.onclick=()=>{ if(confirm("¿Finalizar este mes? Nadie podrá editar hasta reabrir.")){ setMonthLocked(state,true); renderLockButtons(); re(); } };
  btnUnlock.onclick=()=>{ if(confirm("¿Reabrir este mes para edición?")){ setMonthLocked(state,false); renderLockButtons(); re(); } };
  renderLockButtons(); hdrExtra.append(btnLock,btnUnlock);
  const headerEl = header(state,"Panel de Gerencia",monthSwitcher(state,re)); headerEl.appendChild(hdrExtra); app.appendChild(headerEl);

  const container=document.createElement("div"); container.className="container admin-layout";

  // Sidebar (solo en Hojas)
  ...
  tabs.append(t1,t2,t3); main.appendChild(tabs);
  ...
  function mountHojas(){
    view.innerHTML="";
    // Banner estado mes
    const banner=document.createElement('div'); banner.className='no-print'; banner.style.margin='8px 0'; banner.style.padding='8px 12px'; banner.style.border='1px solid #e5e7eb'; banner.style.borderRadius='10px'; banner.style.background='#fff8e1';
    function renderBanner(){ const locked=isMonthLocked(state); banner.textContent = locked ? "Mes FINALIZADO: edición bloqueada hasta reabrir." : "Mes ABIERTO: edición permitida."; }
    renderBanner(); view.appendChild(banner);
    ...
    if(u){
      ...
      scard.appendChild(timesheetTable(state,u,{showObservations:true, obsMode:'admin'}));
    ...
    usersNon.forEach(emp=>{
      ...
      tmp.appendChild(timesheetTable(state,emp,{showObservations:false, obsMode:'admin'}));
  ...
  t1.onclick=()=>{ t1.classList.add("active"); t2.classList.remove("active"); t3.classList.remove("active"); renderMain(); };
  t2.onclick=()=>{ t2.classList.add("active"); t1.classList.remove("active"); t3.classList.remove("active"); renderMain(); };
  t3.onclick=()=>{ t3.classList.add("active"); t1.classList.remove("active"); t2.classList.remove("active"); renderMain(); };
